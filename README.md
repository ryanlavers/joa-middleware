...
Base class for exceptions that can indicate the HTTP status code and error message to be returned to the client
 
Middleware can throw these exceptions as a convenience for indicating common errors, such as `NotFoundException` (404 Not Found)

Requires the `ErrorHandler` middleware to be installed in the chain above any middleware that wishes to throw these exceptions.
